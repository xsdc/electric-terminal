...
module.exports =
class ElectricTerminalView extends View
  animating: false
  id: ''
  maximized: false
  opened: false
  ...
  @content: ->
    @div class: 'electric-terminal terminal-view', outlet: 'electricTerminalView', =>
  ...
  initialize: (@id, @pwd, @statusIcon, @statusBar, @shell, @args=[], @env={}, @autoRun=[]) ->
    ...
    @prevHeight = atom.config.get('electric-terminal.style.defaultPanelHeight')
    ...
    @setAnimationSpeed()
    @subscriptions.add atom.config.onDidChange 'electric-terminal.style.animationSpeed', @setAnimationSpeed

    override = (event) ->
      return if event.originalEvent.dataTransfer.getData('electric-terminal') is 'true'
    ...
    @xterm.on 'mouseup', (event) =>
      if event.which != 3
        ...
        if atom.config.get('electric-terminal.toggles.selectToCopy') and text
    ...
    if /zsh|bash/.test(@shell) and @args.indexOf('--login') == -1 and Pty.platform isnt 'win32' and atom.config.get('electric-terminal.toggles.loginShell')
      @args.unshift '--login'
  ...
  setAnimationSpeed: =>
    @animationSpeed = atom.config.get('electric-terminal.style.animationSpeed')
  ...
  displayTerminal: ->
    ...
    @terminal = new Terminal {
      cursorBlink     : false
      scrollback      : atom.config.get 'electric-terminal.core.scrollback'
      cols, rows
    }
  ...
  attachListeners: ->
    @ptyProcess.on "electric-terminal:data", (data) =>
      @terminal.write data

    @ptyProcess.on "electric-terminal:exit", =>
      @destroy() if atom.config.get('electric-terminal.toggles.autoClose')
    ...
    @ptyProcess.on "electric-terminal:title", (title) =>
      @process = title
    ...
    @terminal.once "open", =>
      ...
      autoRunCommand = atom.config.get('electric-terminal.core.autoRunCommand')
  ...
  open: =>
    ...
    @onTransitionEnd =>
      if not @opened
        ...
        @emit "electric-terminal:terminal-open"
      else
        @focus()
  ...
  pty: () ->
    if not @opened
      wait = new Promise (resolve, reject) =>
        @emitter.on "electric-terminal:terminal-open", () =>
          resolve()
  ...
  ptyPromise: () ->
    new Promise (resolve, reject) =>
      if @ptyProcess?
        @ptyProcess.on "electric-terminal:pty", (pty) =>
          resolve(pty)
  ...
  applyStyle: ->
    config = atom.config.get 'electric-terminal'
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontFamily', (event) =>
      ...
      @terminal.element.style.fontFamily = overrideFont or editorFont or defaultFont
    @subscriptions.add atom.config.onDidChange 'electric-terminal.style.fontFamily', (event) =>
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontSize', (event) =>
      ...
      @terminal.element.style.fontSize = "#{overrideFontSize or editorFontSize}px"
      @resizeTerminalToView()
    @subscriptions.add atom.config.onDidChange 'electric-terminal.style.fontSize', (event) =>
  ...
  insertSelection: (customText) ->
    ...
    runCommand = atom.config.get('electric-terminal.toggles.runInsertedText')
    selectionText = ''
  ...
  getTitle: ->
    @statusIcon.getName() or "electric-terminal"
