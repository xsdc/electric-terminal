...
filteredEnvironment = do ->
  ...
  env.TERM_PROGRAM = 'electric-terminal'
  return env
...
module.exports = (pwd, shell, args, env, options={}) ->
  ...
  emitTitle = _.throttle ->
    emit('electric-terminal:title', ptyProcess.process)
  , 500, true

  ptyProcess.on 'data', (data) ->
    emit('electric-terminal:data', data)
    emitTitle()

  ptyProcess.on 'exit', ->
    emit('electric-terminal:exit')
    callback()
  ...
  process.on 'message', ({event, cols, rows, text}={}) ->
    switch event
      ...
      when 'pty' then emit('electric-terminal:pty', ptyProcess.pty)
