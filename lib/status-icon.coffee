...
class StatusIcon extends HTMLElement
  ...
  initialize: (@terminalView) ->
    @classList.add 'electric-terminal-status-icon'
  ...
  setupTooltip: ->
    ...
    onMouseEnter = (event) =>
      return if event.detail is 'electric-terminal'
      @updateTooltip()
  ...
  updateTooltip: ->
    ...
    @dispatchEvent(new CustomEvent('mouseenter', bubbles: true, detail: 'electric-terminal'))
...
module.exports = document.registerElement('electric-terminal-status-icon', prototype: StatusIcon.prototype, extends: 'li')
