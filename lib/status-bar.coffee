...
ElectricTerminalView = require './view'
...
class StatusBar extends View
  ...
  @content: ->
    @div class: 'electric-terminal status-bar', tabindex: -1, =>
  ...
  initialize: (@statusBarProvider) ->
    ...
    @subscriptions.add atom.commands.add 'atom-workspace',
      'electric-terminal:focus': => @focusTerminal()
      'electric-terminal:new': => @newTerminalView()
      'electric-terminal:toggle': => @toggle()
      'electric-terminal:next': =>
        return unless @activeTerminal
        return if @activeTerminal.isAnimating()
        @activeTerminal.open() if @activeNextTerminalView()
      'electric-terminal:prev': =>
        return unless @activeTerminal
        return if @activeTerminal.isAnimating()
        @activeTerminal.open() if @activePrevTerminalView()
      'electric-terminal:clear': => @clear()
      'electric-terminal:close': => @destroyActiveTerm()
      'electric-terminal:close-all': => @closeAll()
      'electric-terminal:rename': => @runInActiveView (i) -> i.rename()
      'electric-terminal:insert-selected-text': => @runInActiveView (i) -> i.insertSelection('$S')
      'electric-terminal:insert-text': => @runInActiveView (i) -> i.inputDialog()
      'electric-terminal:insert-custom-text-1': => @runInActiveView (i) -> i.insertSelection(atom.config.get('electric-terminal.customTexts.customText1'))
      'electric-terminal:insert-custom-text-2': => @runInActiveView (i) -> i.insertSelection(atom.config.get('electric-terminal.customTexts.customText2'))
      'electric-terminal:insert-custom-text-3': => @runInActiveView (i) -> i.insertSelection(atom.config.get('electric-terminal.customTexts.customText3'))
      'electric-terminal:insert-custom-text-4': => @runInActiveView (i) -> i.insertSelection(atom.config.get('electric-terminal.customTexts.customText4'))
      'electric-terminal:insert-custom-text-5': => @runInActiveView (i) -> i.insertSelection(atom.config.get('electric-terminal.customTexts.customText5'))
      'electric-terminal:insert-custom-text-6': => @runInActiveView (i) -> i.insertSelection(atom.config.get('electric-terminal.customTexts.customText6'))
      'electric-terminal:insert-custom-text-7': => @runInActiveView (i) -> i.insertSelection(atom.config.get('electric-terminal.customTexts.customText7'))
      'electric-terminal:insert-custom-text-8': => @runInActiveView (i) -> i.insertSelection(atom.config.get('electric-terminal.customTexts.customText8'))
      'electric-terminal:fullscreen': => @activeTerminal.maximize()

    @subscriptions.add atom.commands.add '.xterm',
      'electric-terminal:paste': => @runInActiveView (i) -> i.paste()
      'electric-terminal:copy': => @runInActiveView (i) -> i.copy()

    @subscriptions.add atom.workspace.onDidChangeActivePaneItem (item) =>
      return unless item?

      if item.constructor.name is "ElectricTerminalView"
        setTimeout item.focus, 100
      else if item.constructor.name is "TextEditor"
        mapping = atom.config.get('electric-terminal.core.mapTerminalsTo')
        ...
        if prevTerminal != nextTerminal
          if not nextTerminal?
            if atom.config.get('electric-terminal.core.mapTerminalsToAutoOpen')
    ...
    handleBlur = =>
      if terminal = ElectricTerminalView.getFocusedTerminal()
  ...
  registerContextMenu: ->
    @subscriptions.add atom.commands.add '.electric-terminal.status-bar',
      'electric-terminal:status-red': @setStatusColor
      'electric-terminal:status-orange': @setStatusColor
      'electric-terminal:status-yellow': @setStatusColor
      'electric-terminal:status-green': @setStatusColor
      'electric-terminal:status-blue': @setStatusColor
      'electric-terminal:status-purple': @setStatusColor
      'electric-terminal:status-pink': @setStatusColor
      'electric-terminal:status-cyan': @setStatusColor
  ...
  registerPaneSubscription: ->
    @subscriptions.add @paneSubscription = atom.workspace.observePanes (pane) =>
      ...
      tabBar.on 'dragstart', (event) ->
        return unless event.target.item?.constructor.name is 'ElectricTerminalView'
        event.originalEvent.dataTransfer.setData 'electric-terminal-tab', 'true'
      pane.onDidDestroy -> tabBar.off 'drop', @onDropTabBar

  createTerminalView: (autoRun) ->
    shell = atom.config.get 'electric-terminal.core.shell'
    shellArguments = atom.config.get 'electric-terminal.core.shellArguments'
    args = shellArguments.split(/\s+/g).filter (arg) -> arg
    shellEnv = atom.config.get 'electric-terminal.core.shellEnv'
    env = {}
  ...
  createEmptyTerminalView: (autoRun=[], shell = null, args = [], env= {}) ->
    ...
    switch atom.config.get('electric-terminal.core.workingDirectory')
    ...
    electricTerminalView = new ElectricTerminalView(id, pwd, statusIcon, this, shell, args, env, autoRun)
    statusIcon.initialize(electricTerminalView)

    electricTerminalView.attach()

    @terminalViews.push electricTerminalView
    @statusContainer.append statusIcon
    return electricTerminalView
  ...
  onDragOver: (event) =>
    ...
    unless event.originalEvent.dataTransfer.getData('electric-terminal') is 'true'
      return
  ...
  onDrop: (event) =>
    ...
    panelEvent = dataTransfer.getData('electric-terminal-panel') is 'true'
    tabEvent = dataTransfer.getData('electric-terminal-tab') is 'true'
  ...
  onDropTabBar: (event, pane) =>
    {dataTransfer} = event.originalEvent
    return unless dataTransfer.getData('electric-terminal-panel') is 'true'
